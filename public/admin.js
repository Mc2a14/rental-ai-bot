...
setupRealTimeValidation() {
    ...
    requiredFields.forEach(field => {
        ...
        if (!validationMsg && field.id === 'propertyType') {
            // Special handling for property type
            validationMsg = document.createElement('div');
            validationMsg.className = 'validation-message';
            validationMsg.textContent = 'Please select a property type';
        ...
        field.addEventListener('input', validateThisField);
        field.addEventListener('change', validateThisField);
...
validateField(field) {
    ...
    if (field.tagName === 'SELECT') {
        ...
        console.log(`🔍 Select field "${field.id}" validation: ${selectIsValid ? 'VALID' : 'INVALID'} (value: "${field.value}")`);
...
autoLoadExistingConfig() {
    ...
    try {
        ...
        if (!savedConfig && !savedAppliances && !savedRecommendations) {
            ...
            // Set default property type to "Vacation Home"
            setTimeout(() => {
